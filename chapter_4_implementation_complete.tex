...
\subparagraph{Hyperparameters.} The privacy hyperparameters are held identical across Step 1 and Step 3, so that the two rounds are directly comparable and their budgets add without further analysis. The exception is the expected batch size: Step 1 uses the public number of items per complete cycle, $s_1=B$, while Step 3 uses the public minimum number of items required for a label, $s_3=X$. The values in Table \ref{tab:dp-hyperparameters} are the settings of the pilot configuration and are for a theoretical, rather than practical proof-of-concept. The $\varepsilon$ in particular is set far above any privacy-meaningful level.

The protected example is one memory item rather than one trajectory. Each trajectory produces between one and three ReasoningBank memory items, and these are flattened before either private-prediction round. A trajectory containing three memories therefore contributes three separate Gemma prompt rows and counts as three examples in the privacy mechanism. The guarantee is consequently add-or-remove-one-item differential privacy, and not user-level privacy.
...
$\delta$ & Privacy $\delta$ per round & $10^{-5}$ \\
\hline
\end{tabular}
...
The number of labels generated per cycle is 80. A complete cycle contains exactly the configurable public count $B$ of new memory items. Step 1 therefore uses $s_1=B$, regardless of how many trajectories produced those items. Fewer than $B$ available items remain pending until a complete cycle can be formed.
...
The threshold $X$, the minimum number of memory items a label must attract before a shared memory is generated from it, is set to 5. Every item in a bucket that does not reach five is written separately to a carry-over file and re-enters the mechanism at the next cycle.

\subparagraph{Step 3 - Number of memories generated per label.}
The mechanism produces exactly one shared memory per qualifying label per cycle. Every item assigned to a qualifying label is passed to Gemma as a separate prompt row, including when the realised bucket contains more than $X$ items. The expected batch size used for Step 3 accounting remains the public value $s_3=X$ rather than the realised private count. While the Amin et al. paper runs Algorithm 1 repeatedly to build up a dataset of many synthetic examples from the same private corpus, this thesis focuses on the simplest implementation with one memory. The number of shared memories available to the agent grows across cycles instead, as new private memories accumulate and new labels are synthesised.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{} X @{}}
        ...
        \midrule
        Return exactly {k} labels that summarise the task query below. Each label must be 1–4 words and on a DISTINCT facet — no two labels may be synonyms, paraphrases, or describe the same aspect. Return only a JSON array of strings.
