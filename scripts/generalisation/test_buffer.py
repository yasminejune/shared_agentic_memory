...
pure logic over integer ``entry_index`` lists, and the persistence
layer is straight JSONL round-tripping of
:class:`agent_memories.memory.MemoryEntry`. This is the Phase 1.3
...
def _fake_entry(user_id: str, query: str, *, dim: int = 4) -> MemoryEntry:
    """Construct a deterministic :class:`MemoryEntry` for buffer tests.
...
def _run_roundtrip(path: Path) -> bool:
    ...
    for entry in loaded:
        print(
            ...
            f"outcome={entry.outcome} items={len(entry.items)} "
            f"embedding_dim={len(entry.embedding) if entry.embedding else 0}"
        )
    return matched
...
def _run_orchestrator_glue(path: Path) -> None:
    ...
    print(f"Carry-over entry_indices: {result.carry_over}")
