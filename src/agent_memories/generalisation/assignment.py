"""WP2 round-2 batch assignment: per-memory nearest-label by cosine.
...
memory to its nearest label. The DP guarantee for round-2 batch
...
label embeddings, not the ``N`` memory embeddings.
"""
...
class LabelAssignment:
    """Result of :func:`assign_memories_to_labels` for one memory.

    ``entry_index`` is the position of the memory in the input list
...
def assign_memories_to_labels(
    ...
    *,
    ...
    """Assign each ``MemoryEntry`` to its nearest label by cosine similarity.
    ...
    over the memory's ``query`` field via the same embedder model
    ...
    entries with an embedding; a missing one would be a corruption,
    ...
    for idx, entry in enumerate(memories):
        if entry.embedding is None:
            raise ValueError(
                f"Memory at index {idx} (user_id={entry.user_id!r}, "
                f"query={entry.query!r}) has no embedding; the WP1.6 "
                "store should never write entries without one."
            )
...
def group_by_label(
    ...
    Returns ``n_labels`` lists, each holding the ``entry_index`` of
    every memory assigned to that label, in the order the memories
    ...
    for assignment in assignments:
        if 0 <= assignment.label_index < n_labels:
            buckets[assignment.label_index].append(assignment.entry_index)
    return buckets
